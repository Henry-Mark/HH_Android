...
import com.henry.hh.utils.SpringSystem;
...
public class JellyViewPager extends ViewPager {
	 ...
	 *            滑动速度
	 */
	private void animOutIfNeeded(float scrollDis, float velocityX) {
		float tranX = 0;
		tranSpring.setOvershootClampingEnabled(true);
		// 右移
		if (velocityX > FLING_VELOCITY || (scrollDis > OUT_DISTANCE_BOUDARY)) {
			if (hasNext()) {
				tranX = mWidth;
